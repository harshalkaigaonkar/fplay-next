...
import { selectCurrentSongId, selectSongsQueue } from 'redux/slice/playerSlice';
...
const BottomSheet: FC<{ children: JSX.Element }> = ({ children }) => {
	...
	const [currentTrack, setCurrentTrack] = useState<SaavnSongObjectTypes>(
		songsQueue.find((item: SaavnSongObjectTypes) => item.id === currentSongId),
	);

	const onPanelControl = () => {
		if (bottomSheet) {
			dispatch(onClosePanel());
		} else dispatch(onOpenPanel());
	};
	return (
		<span className='animate-enter-opacity select-none'>
			...
			<section
				className={` ${bottomSheet ? 'h-[42rem] animate-enter-bottom' : ''}
            fixed max-h-[42rem] min-h-[2rem] bottom-0 lg:left-20 lg:right-20 md:left-0 md:right-0 bg-black z-40 rounded-xl border-solid border-white`}>
				<span className='absolute -mt-10 -ml-10 appearance-none w-full inline-flex justify-end items-center cursor-pointer transition duration-300 z-30'>
					<button
						className='h-fit bg-black text-white border-solid border-white focus:outline-none rounded-full inline-flex items-center'
						onClick={onPanelControl}>
						<ChevronDownIcon

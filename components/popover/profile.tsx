		...
		<Transition
			...
			<ul className='absolute w-60 min-w-40 max-w-60 z-10 top-11 -left-20 list-none px-0 bg-[#343434] text-white text-lg rounded-md shadow-lg'>
				<header className='my-1 px-5 py-3 border-b-1 border-t-0 border-l-0 border-r-0 border-solid border-[#7a7a7a]'>
					<h5 className='h-fit min-w-full overflow-hidden truncate'>
						{session && session.user.name}
					</h5>
					<h6 className='h-fit p-0 m-0 max-w-full overflow-hidden text-ellipsis font-normal'>
						{session && session.user.email}
					</h6>
				</header>
				<li className='m-2 flex flex-row gap-4 items-center p-2 rounded-md hover:bg-[#212121] '>
					<>
						<UserCircleIcon className='w-6 h-6 m-2' />
					</>
					<>
						<h4>Profile</h4>
					</>
				...
				<li className='m-2 flex flex-row gap-4 items-center p-2 rounded-md hover:bg-[#212121] '>
					<>
						<MusicalNoteIcon className='w-6 h-6 m-2' />
					</>
					<>
						<h4>Library</h4>
					</>

...
const PanelSongResult: React.FC<{
	data: any;
	key: number;
	audioElement?: MutableRefObject<HTMLAudioElement | null>;
}> = ({ data, key, audioElement }) => {
	...
	return (
       ...
       rounded-full transition duration-300 cursor-pointer`}
			...
			<span
				className={`${
					mouseEnter || (currentSongId === data.id && !paused)
						? 'w-3/12 '
						: 'w-2/3'
				} h-fit content-center`}>
				<p className='text-sm font-bold cursor-pointer truncate'>
					{decodeHTMLContent(data?.title ?? data.name)}
				</p>
				<p className='mt-1 text-[10px] font-normal cursor-pointer truncate'>
					{decodeHTMLContent(
						data?.description ??
							`${data.primaryArtists} ${
			...
			{mouseEnter ? (
				<div className='mx-[1px] flex-1 flex flex-row items-center justify-evenly animate-enter-div-1'>
					...
					{!songsQueue?.find(
						(song: SaavnSongObjectTypes) => song.id === data.id,
					) ? (
						<button
							className='p-2 h-fit rounded-full bg-black inline-flex items-center cursor-pointer hover:bg-black/30 active:bg-black/60'
						...
						<button className='p-2 h-fit rounded-full bg-black inline-flex items-center cursor-default'>

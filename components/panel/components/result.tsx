...
import {
	SaavnAlbumObjectTypes,
	SaavnArtistObjectTypes,
	SaavnSongObjectTypes,
} from 'types';
import {
	fetchAlbumInfoThroughId,
	fetchAlbumsThroughSearchQuery,
} from 'helpers/music/fetchAlbums';
import {
	fetchArtistInfoThroughId,
	fetchArtistSongsThroughId,
	fetchArtistsThroughSearchQuery,
} from 'helpers/music/fetchArtists';
				...
				return (
					<>
						<InfoOverlay
							backHandler={overlayBackHandler}
							overlayType={overlayType}
							error={fetchingError}></InfoOverlay>
					</>
				);
		}
									...
									case 'artist':
										return (
											<PanelArtistResult
												...
												key={index}
												audioElement={audioElement}
												onClickHandler={onArtistClickHandler}
											/>
										);
									case 'album':
										return (
			...
			{!!data?.albums?.results && data?.albums?.results?.length > 0 && (
				<div className='w-full flex flex-col'>
					...
					<span className='mt-2 flex justify-between overflow-y-auto overflow-x-hidden'>
						{!!data?.albums &&
							data?.albums?.results?.map((data: any, index: number) => (
								<>
									<PanelAlbumResult
										data={data}
										key={index}
										audioElement={audioElement}
										onClickHandler={onAlbumClickHandler}
									/>
								</>
							))}
					</span>

      ...
      <section className='mt-5 px-3'>
        <div className=' flex justify-start gap-5'>
          ...
          <div className='w-auto flex-1 flex flex-col items justify-center'>
            ...
            <h5 className='mt-3 font-normal text-md'>{decodeHTMLContent(`${result.year} · ${result.primaryArtists}${result.featuredArtists && result.featuredArtists.length > 0 ? ` ft. ${result.featuredArtists}`: ""} · ${result.songs.length} Song${result.songs.length > 1 ? 's':''}`)}</h5>
          </div>
        </div>
        <div className='mt-4 flex flex-col gap-5'>
          ...
          <span className='lg:h-[15rem] flex flex-row flex-wrap overflow-y-auto overflow-x-hidden'>
            {!!result?.songs && result?.songs?.map((item: SaavnSongObjectTypes, index: number) => 

			...
			{!!currentTrack ? (
				<>
					<span className='w-full h-fit inline-flex flex-row items-center justify-evenly'>
						<span
							onClick={onPrevTrack}
							className='p-2 inline-flex items-center h-fit text-white/70 rounded-full hover:-translate-x-1 hover:bg-white/5 cursor-pointer transition duration-500 active:bg-[#121212]'>
						...
						<div className='flex-0 flex flex-row justify-center items-center gap-10 border-1 border-white/50 border-solid rounded-2xl shadow-2xl overflow-hidden group-hover:scale-105'>
							<Image
								src={
									currentTrack?.image?.[2]?.link ??
									currentTrack?.image?.[1]?.link ??
									currentTrack?.image?.[0]?.link ??
					...
					<span className='w-96 px-12 my-2'>
						<h1 className='lg:w-full truncate oveflow-hidden font-bold text-center'>
							{decodeHTMLContent(currentTrack.name)}
						</h1>
						<h5 className='lg:mt-2 lg:w-full truncate oveflow-hidden font-normal text-center'>
							{decodeHTMLContent(
								`${currentTrack.primaryArtists}${currentTrack.featuredArtists && ` ft. ${currentTrack.featuredArtists}`}`,
							)}
						</h5>
					</span>
				</>

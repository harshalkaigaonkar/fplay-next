...
export const playerSlice = createSlice({
	...
	reducers: {
		...
		onSetupPlayer: (state, action): void => {
			const { songsQueue, currentSongId, paused, time } = action.payload;
		...
		onAddSongIntoQueue: (
			...
			action: PayloadAction<
				SaavnSongObjectTypes | SaavnSongObjectTypes[] | any
			>,
		) => {
			if (
				action.payload.length &&
				state.songsQueue.find((item: SaavnSongObjectTypes) => {
					return action.payload.find(
						(addItem: SaavnSongObjectTypes) => addItem.id === item.id,
					)
						? true
						: false;
				})
			...
			if (state.songsQueue.length === 0 || !state.currentSongId)
				state.currentSongId = updatedSongsQueue[0].id;
		...
		onRemoveSongFromQueue: (
		...
		): SaavnSongObjectTypes[] | any => {
			if (
				!state.songsQueue.find(
					(item: SaavnSongObjectTypes) => item.id === action.payload,
				)
			)
				console.log({
					type: 'error',
			...
			state.songsQueue = state.songsQueue.filter(
				(item: SaavnSongObjectTypes) => item.id !== action.payload,
			);
		},
		/**
		...
		onReaarrangeSongQueue: (
			state,
			action: PayloadAction<{
				indexReplacedFrom: number;
				indexReplacedTo: number;
			}>,
		): SaavnSongObjectTypes[] | any => {
			if (
				state.songsQueue.length - 1 < action.payload.indexReplacedFrom ||
				state.songsQueue.length - 1 < action.payload.indexReplacedTo
			)
				return;
			let updatedSongsQueue = state.songsQueue;
			const [removed] = updatedSongsQueue.splice(
				action.payload.indexReplacedFrom,
				1,
			);
		...
		onChangeNextSongFromQueue: (state) => {
			const index = state.songsQueue.findIndex(
				(item) => item.id === state.currentSongId,
			);
